...
foreach($featureName in $featureNames) {
    try {
        ...
        Write-Host "Successfully Disabled the feature: $featureName"
    }
    catch {
        Write-Error "Failed to disable feature: $featureName. Error: $($_.Exception.Message)"
    }
}
...
if ($featureStatus.RestartNeeded) {
    Write-Host "A restart is required to complete the disablement of $featureName."
